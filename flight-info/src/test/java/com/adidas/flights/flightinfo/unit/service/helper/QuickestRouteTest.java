...
public class QuickestRouteTest {

	private QuickestRoute quickestRoute;
	...
	public void shouldReturnOneRoute() {
		...
		frankfurt.getDestinations().add(new Destination(glasgow, 21000L));
		frankfurt.getDestinations().add(new Destination(london, 20000L));
		
		quickestRoute = new QuickestRoute("Zaragoza", "Glasgow");
		List<RouteDto> routes = quickestRoute.getInfo(flights);
		
		assertThat(routes,hasSize(1));
		RouteDto route = routes.get(0);
		assertTrue(route.getTime() == 31000L);
